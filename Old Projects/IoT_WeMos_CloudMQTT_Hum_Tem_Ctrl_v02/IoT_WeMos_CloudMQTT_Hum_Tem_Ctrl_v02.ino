...
unsigned int error_COUNT = 0; //Counter for sensor's measurements error
bool sensor_STAT = 0; //Flag for sesnor status (current error status)
bool sensor_MEM = 0; //Flag for sesnor error memory - if measure fails, this flags turns to 1 and remains until reset
...
void reconnect_mqtt()
{
  ...
  Serial.println();
  Serial.println();
  Serial.print("Trying to reconnect to MTQQ Broker... ");
  Serial.println(mqttServer);
...
void publish_readings()
{
  ...
  // Sensor parameters
  JSONencoder["SENSSTAT"] = String (sensor_STAT);
  JSONencoder["SENSMEM"] = String (sensor_MEM);
  JSONencoder["ERRCOUNT"] = String (error_COUNT);
...
void update_WeMos (String inputmsg)
{
  ...
  else
  {
    ...
    if (reset_SYS)
    {
      Serial.println("RESET request received - Shooting-down and resetting WeMos SW/HW.");
      delay(100);
      Serial.println("RESET request received - Shooting-down and resetting WeMos SW/HW..");
      delay(100);
      Serial.println("RESET request received - Shooting-down and resetting WeMos SW/HW...");
...
void print_sensor()
{
  ...
  Serial.print("Current sensor's status (actual/measured): "); //Print the auxiliar text: "Current sensor's status (actual/measured): " to Serial Comm (USB)
  Serial.println(sensor_STAT); //Print the current value of the stat variable to Serial Comm (USB)
  Serial.print("Memory sensor's status: "); //Print the auxiliar text: "Memory sensor's status: " to Serial Comm (USB)
  Serial.println(sensor_MEM); //Print the current value of the mem variable to Serial Comm (USB)
  Serial.print("Sensor's error counter: "); //Print the auxiliar text: "Memory sensor's counter: " to Serial Comm (USB)
  Serial.println(error_COUNT); //Print the current value of the count variable to Serial Comm (USB)
}
